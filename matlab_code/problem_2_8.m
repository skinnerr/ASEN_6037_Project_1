function [] = problem_2_8( HIT, save_plots )
    ...
    data_min = min(min(enstrophy_norm(:,:,128)));
    data_max = max(max(enstrophy_norm(:,:,128)));
    ...
    image_data = squeeze(enstrophy_norm(:,:,128))';
    ...
    ylabel(hbar,'\Omega / \langle\Omega\rangle_{xyz}');

    if save_plots
        % Save figures to file (dpdf = PDF file) (loose = uncropped)
        filename = '../images/prob2_8.pdf';
