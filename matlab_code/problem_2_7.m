...
% Calculates the velocity gradient tensor (A_ij = d u_i / d x_j) for HIT.
    ...
    % Calculate volume average of A_ij.
    Aij_squared_mean = zeros(3,3);
    for i = 1:3
    for j = 1:3
        Aij_squared_mean(i,j) = mean(mean(mean(Aij_squared(i,j,:,:,:))));
    end
    end
    
    % Report volume average of A_ij.
    fprintf('Volume average of A_{ij}^2 is as follows:\n');
    for i = 1:3
        fprintf('i = %1i, j = {1,2,3}:',i)
        for j = 1:3
            fprintf('   %+10.4e',Aij_squared_mean(i,j));
        end
        fprintf('\n');
    end
    ...
    Aij_symm = Aij .* permute(Aij,[2,1,3,4,5]);
    
    % Calculate volume average of A_ij.
    Aij_symm_mean = zeros(3,3);
    for i = 1:3
    for j = 1:3
        Aij_symm_mean(i,j) = mean(mean(mean(Aij_symm(i,j,:,:,:))));
    end
    end
    
    % Report volume average of A_ij.
    fprintf('Volume average of 2*A_{ij}*A_{ji} is as follows:\n');
    for i = 1:3
        fprintf('i = %1i, j = {1,2,3}:',i)
        for j = 1:3
            fprintf('   %+10.4e',2*Aij_symm_mean(i,j));
        end
        fprintf('\n');
    end
